...
class Policy(torch.nn.Module):
    ...
    def forward(self, x):
        """ map the observation x = s(t) from the environment at time-step t into 
...
class A2CPolicy:
    ...
    def __init__(self, state_space: int, action_space: int,
                 seed: int = 42, **kwargs):
        """ initialize the multi-layer neural networks for the actor and the critic
...
class A2C:
    ...
    def __init__(self, policy,
                 ...
                 seed: int = 42,
                 **kwargs):
        """ initialize an agent to learn a policy via Advantage Actor-Critic algorithm 
    ...
    def predict(self, obs, state=None, episode_start=None, deterministic=False):
        """ predict an action based on observation s(t) and policy π(a(t)|s(t))
