...
class Callback():
    def __init__(self, agent, env, args):
        """ initialize a callback object to access 
...
def display(frame, step, episode, reward):
    """ render the (state -> action) step in the target environment
...
def multiprocess(args, train_env, test_env, train, seeds):
    """ execute multiple sequential training and testing sessions 
...
def stack(args, metric, records):
    """ stack training sessions outputs 
...
def track(metric, xs, ys, sigmas, args, label, filename):
    """ plot the RL agent's performance in the target environment 
    (by reference to the evaluation metric) over training episodes
...
def collect(env, seed, maxit=10):
    """ collect trajectories in the environment """
    data = list()
    env.seed(seed)
    num_episodes = 0
...
def optimize_params(real_data, sim_data, seed,
    ...
    """ optimize the physical parameters (masses) of a simulation environment 
